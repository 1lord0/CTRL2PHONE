...
export function getVirtualBounds(displays: { bounds: Rect }[]): Rect {
  if (displays.length === 0) {
    return { x: 0, y: 0, width: 0, height: 0 };
  }
  const first = displays[0].bounds;
  const acc = displays.reduce(
    (a, d) => ({
...
export function clampRectToDisplay(rect: Rect, displayBounds: Rect): Rect {
...
export function computeCropRect(
  ...
  if (!isFinite(scaleX) || scaleX <= 0) scaleX = fallback;
  if (!isFinite(scaleY) || scaleY <= 0) scaleY = fallback;
  return {
