...
export interface DragProxyCallbacks {
  onReady: () => void;
  onStarting: (confirmGo: () => void) => void;
  onStarted?: () => void;
  onDone: (effect: string) => void;
  onFailed: (reason: string) => void;
}

export class SelectionDragProxy {
  private processPort: ChildProcessPort;
  private callbacks: DragProxyCallbacks;
  private sessionId: number;
  private generation: number;
  private readyTimeout: NodeJS.Timeout | null = null;
  private isReady = false;
  private isCleanedUp = false;
  private hasStartingFired = false;
  private hasGoWritten = false;
  private hasStartedFired = false;

  constructor(
    processPort: ChildProcessPort,
    callbacks: DragProxyCallbacks,
    sessionId: number,
    generation: number
  ) {
  ...
  private handleLine(rawLine: string): void {
    ...
    if (line === 'STARTING') {
      if (!this.isReady) return;
      if (this.hasStartingFired) return;
      this.hasStartingFired = true;

      this.callbacks.onStarting(() => {
        if (this.isCleanedUp) return;
        if (this.hasGoWritten) return;
    ...
    if (line === 'STARTED') {
      if (!this.hasStartingFired) return;
      if (this.hasStartedFired) return;
