...
export interface ActivationRequest {
  windowPort: OverlayWindowPort;
  bounds: Rect;
  selectionRect: Rect | null;
  backgroundImagePath: string;
  sessionId: number;
  waitForReady: () => Promise<void>;
  prepareRenderWaiter: (sessionId: number) => void;
  waitForRendered: (sessionId: number) => Promise<void>;
  isCurrent: () => boolean;
}

export async function activateSelectionOverlay(request: ActivationRequest): Promise<void> {
