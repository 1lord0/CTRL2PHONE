...
const en = {
    ...
    'hero.lede': 'Double-tap Left Ctrl to open the selection overlay. After drawing a region: X or Enter sends it to your AI provider, M sends it to your phone. Press Esc to cancel, Q to quit.\nCtrl+Shift+V → Send clipboard text/link to phone instantly.',
    ...
    'hint.aiProvider': 'In "Web" mode the image is pasted into gemini.google.com. Pick an API provider and enter a key, and the selected region is sent to the API with the reply shown right here.',
    ...
    'hint.setupRls': 'One-time: press the button → the SQL is copied to your clipboard and the Supabase SQL Editor opens → paste it and press Run. It makes the bucket private and scopes the anon key to this bucket only.',
    'label.status': 'Status',
    'status.ready': 'Ready',
    ...
    'response.rlsPrefix': 'The SQL below was copied - paste it into the Supabase SQL Editor and press Run:\n\n',
    'confirm.purge': 'All images in the Supabase bucket (including to_pc) will be PERMANENTLY deleted. Are you sure?',
    ...
    'hint.pillVisibility': 'Background mode hides the pill; use Ctrl+Shift+Space to open the panel. Hotkeys still work.',
...
const tr = {
    ...
    'hero.lede': "İki kere üst üste sol Ctrl'e basınca seçim katmanı açılır. Fareyle alanı çizdikten sonra: X veya Enter ile yapay zeka sağlayıcına, M ile Telefona gönderirsiniz. İptal etmek için Esc tuşuna basın. Q ile çık.\nCtrl+Shift+V → Panodaki metni/linki anında telefona gönderir.",
    ...
    'ph.aiBaseUrl': 'http://localhost:11434/v1',
    'hint.aiProvider': '"Web" modunda görsel gemini.google.com\'a yapıştırılır. Bir API sağlayıcı seçip anahtar girersen, seçtiğin alan API\'ye gönderilir ve yanıt doğrudan burada gösterilir.',
    ...
    'hint.setupRls': "Tek seferlik: butona bas → SQL panoya kopyalanır ve Supabase SQL Editör açılır → yapıştırıp Run de. Bucket'ı gizli yapar, anon anahtarını yalnızca bu bucket ile sınırlar.",
    'label.status': 'Durum',
    'status.ready': 'Hazır',
    ...
    'status.rlsCopied': "RLS SQL panoya kopyalandı. Açılan Supabase SQL Editör'e yapıştırıp Run deyin.",
    ...
    'response.rlsPrefix': "Aşağıdaki SQL panoya kopyalandı - Supabase SQL Editör'e yapıştırıp Run deyin:\n\n",
    'confirm.purge': 'Supabase storage bucket içerisindeki tüm görseller (to_pc dahil) KALICI OLARAK silinecektir. Emin misiniz?',
    ...
    'hint.pillVisibility': 'Arka plan modunda pill gizlenir; panel için Ctrl+Shift+Space kullanın. Kısayollar çalışmaya devam eder.',
